#!/usr/bin/env python3
"""
Deployment script for CreditAdvisor BigQuery Data Agent using geminidataanalytics.googleapis.com API.
"""
...
def deploy_credit_advisor_agent():
    project_id = os.environ.get("GOOGLE_CLOUD_PROJECT", "data-agents-by-industry")
    location = os.environ.get("GOOGLE_CLOUD_LOCATION", "global")
    agent_id = "credit-advisor-agent"

    print(f"Deploying CreditAdvisor Data Agent to Project: '{project_id}', Location: '{location}', Agent ID: '{agent_id}'...")

    # Load payload configuration
    payload_path = os.path.join(os.path.dirname(__file__), "create_agent_payload.json")
    ...
    if response.status_code in [200, 201]:
        print(f"✅ Successfully created CreditAdvisor Data Agent! (ID: {agent_id})")
        ...
        if patch_response.status_code in [200, 201]:
            print(f"✅ Successfully updated CreditAdvisor Data Agent configuration! (ID: {agent_id})")
