-- ============================================================================
-- Schema DDL for PulseChecker - Santé, Urgences, RPPS & Biologie Médicale
-- Dataset: healthcare_medical_ds (Project: data-agents-by-industry)
-- Relational Architecture linking Official FINESS French Hospitals,
-- RPPS Medical Demographics, Emergency Rooms, Operating Blocks, Drug Shortages,
-- Staff Plannings, and Official Assurance Maladie Open BIO Expenses.
-- ============================================================================

CREATE SCHEMA IF NOT EXISTS `healthcare_medical_ds`
OPTIONS (
  location = 'US',
  description = 'Dataset PulseChecker : Intelligence medicale, demographie RPPS, opportunites de cliniques, urgences, lits, pharmacie et Open BIO.'
);
...
-- 2. Table: demographie_medecins_rpps (Medical Demographics & Medical Desert Index)
CREATE OR REPLACE TABLE `healthcare_medical_ds.demographie_medecins_rpps` (
  ...
  nom_region STRING OPTIONS(description="Région administrative"),
  population_totale INT64 OPTIONS(description="Population totale du département"),
  nombre_medecins_generalistes INT64 OPTIONS(description="Nombre de médecins généralistes libéraux et salariés inscrits au RPPS"),
  nombre_medecins_specialistes INT64 OPTIONS(description="Nombre de médecins spécialistes répertoriés"),
  densite_medecins_pour_100k_hab NUMERIC OPTIONS(description="Nombre total de médecins pour 100 000 habitants"),
  indice_tension_desert_medical STRING OPTIONS(description="Indicateur de désert médical : CRITIQUE_DESERT_MEDICAL, TENDU, CONFORME"),
  opportunite_implantation_clinique STRING OPTIONS(description="Recommandation d'implantation : PRIORITE_FORTE_INVESTISSEMENT, MOYENNE, FAIBLE"),
  ebitda_projete_5ans_eur NUMERIC OPTIONS(description="EBITDA projeté sur 5 ans en Euros (€) pour une nouvelle clinique de soins")
)
OPTIONS (
  description = "Analyse RPPS de la densité médicale par territoire, détection des déserts médicaux et opportunités d'implantation de cliniques."
);

-- 3. Table: assurance_maladie_open_bio_depenses (Official Ameli Open Bio Medical Biology Expenses)
...
-- 4. Table: hopitaux_flux_admissions_urgences (Emergency Room Admissions, Wait Times & Plan Blanc)
...
-- 5. Table: hopitaux_blocs_operatoires_chirurgie (Surgical Operating Rooms & ICU Bed Capacity)
...
-- 6. Table: pharmacie_stock_medicaments_tension (Hospital Pharmacy Stocks & Critical Drug Shortages)
...
-- 7. Table: personnel_medical_garde_planning (Medical Staffing On-Call Plannings & Absenteeism)
