#!/usr/bin/env python3
"""
Deployment script for TransitNavigator BigQuery Data Agent using geminidataanalytics.googleapis.com API.
"""
...
def deploy_transit_navigator_agent():
    project_id = os.environ.get("GOOGLE_CLOUD_PROJECT", "data-agents-by-industry")
    location = os.environ.get("GOOGLE_CLOUD_LOCATION", "global")
    agent_id = "transit-navigator-agent"

    print(f"Deploying TransitNavigator Data Agent to Project: '{project_id}', Location: '{location}', Agent ID: '{agent_id}'...")

    payload_path = os.path.join(os.path.dirname(__file__), "transit_navigator_payload.json")
    ...
    if response.status_code in [200, 201]:
        print(f"✅ Successfully created TransitNavigator Data Agent! (ID: {agent_id})")
        ...
        if patch_response.status_code in [200, 201]:
            print(f"✅ Successfully updated TransitNavigator Data Agent configuration! (ID: {agent_id})")
        else:
...
if __name__ == "__main__":
    deploy_transit_navigator_agent()
