#!/usr/bin/env python3
"""
Deployment script for ArenaManager BigQuery Data Agent using geminidataanalytics.googleapis.com API.
"""
...
def deploy_arena_manager_agent():
    project_id = os.environ.get("GOOGLE_CLOUD_PROJECT", "data-agents-by-industry")
    location = os.environ.get("GOOGLE_CLOUD_LOCATION", "global")
    agent_id = "arena-manager-agent"

    print(f"Deploying ArenaManager Data Agent to Project: '{project_id}', Location: '{location}', Agent ID: '{agent_id}'...")

    payload_path = os.path.join(os.path.dirname(__file__), "arenamanager_payload.json")
    ...
    if response.status_code in [200, 201]:
        print(f"✅ Successfully created ArenaManager Data Agent! (ID: {agent_id})")
        ...
        if patch_response.status_code in [200, 201]:
            print(f"✅ Successfully updated ArenaManager Data Agent configuration! (ID: {agent_id})")
        else:
