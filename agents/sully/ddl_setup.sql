...
CREATE OR REPLACE TABLE `public_sector_employment_ds.bmo_recrutement_2025` (
  ...
  nom_region STRING OPTIONS(description="Nom de la région administrative (ex: Île-de-France, Auvergne-Rhône-Alpes)"),
  code_departement STRING OPTIONS(description="Code département (ex: 75, 69, 13)"),
  ...
  nom_bassin_emploi STRING OPTIONS(description="Nom du bassin d'emploi (ex: Bassin de Paris, Bassin de Lyon)"),
...
CREATE OR REPLACE TABLE `public_sector_employment_ds.entreprises_urssaf_declarations` (
  ...
  department_code STRING OPTIONS(description="Code département (ex: 75, 69)"),
...
CREATE OR REPLACE TABLE `public_sector_employment_ds.france_travail_demandeurs` (
  ...
  niveau_etudes STRING OPTIONS(description="Niveau de diplôme (Bac, Bac+2, Bac+5, Doctorat)")
)
OPTIONS (
...
CREATE OR REPLACE TABLE `public_sector_employment_ds.candidatures_postulations_suivi` (
  ...
  current_status STRING OPTIONS(description="Statut dans le funnel ATS (Candidature Transmise, Entretien RH, Offre d'Embauche, Embauché)"),
...
CREATE OR REPLACE TABLE `public_sector_employment_ds.france_travail_formations_aides` (
  ...
  nom_aide_dispositif STRING OPTIONS(description="Dispositif public mobilisé (AFPR, POEI, PMSMP Immersion, Aid'Emploi ZFU, AGEFIPH)"),
  ...
  organisme_financeur STRING OPTIONS(description="Organisme financeur (France Travail, Région, Opco, Agefiph)")
)
OPTIONS (
