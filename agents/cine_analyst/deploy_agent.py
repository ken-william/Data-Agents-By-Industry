#!/usr/bin/env python3
"""
Deployment script for CineAnalyst BigQuery Data Agent using geminidataanalytics.googleapis.com API.
"""
...
def deploy_cine_analyst_agent():
    project_id = os.environ.get("GOOGLE_CLOUD_PROJECT", "data-agents-by-industry")
    location = os.environ.get("GOOGLE_CLOUD_LOCATION", "global")
    agent_id = "cine-analyst-agent"

    print(f"Deploying CineAnalyst Data Agent to Project: '{project_id}', Location: '{location}', Agent ID: '{agent_id}'...")

    payload_path = os.path.join(os.path.dirname(__file__), "cineanalyst_payload.json")
    ...
    if response.status_code in [200, 201]:
        print(f"✅ Successfully created CineAnalyst Data Agent! (ID: {agent_id})")
        ...
        if patch_response.status_code in [200, 201]:
            print(f"✅ Successfully updated CineAnalyst Data Agent configuration! (ID: {agent_id})")
        else:
