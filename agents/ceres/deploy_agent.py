#!/usr/bin/env python3
"""
Deployment script for Ceres BigQuery Data Agent using geminidataanalytics.googleapis.com API.
"""
...
def deploy_ceres_agent():
    project_id = os.environ.get("GOOGLE_CLOUD_PROJECT", "data-agents-by-industry")
    location = os.environ.get("GOOGLE_CLOUD_LOCATION", "global")
    agent_id = "ceres-agent"

    print(f"Deploying Ceres Data Agent to Project: '{project_id}', Location: '{location}', Agent ID: '{agent_id}'...")

    # Load payload configuration
    payload_path = os.path.join(os.path.dirname(__file__), "ceres_payload.json")
    ...
    if response.status_code in [200, 201]:
        print(f"✅ Successfully created Ceres Data Agent! (ID: {agent_id})")
        ...
        if patch_response.status_code in [200, 201]:
            print(f"✅ Successfully updated Ceres Data Agent configuration! (ID: {agent_id})")
